package ca.triagebot.triagebot;

import androidx.lifecycle.ViewModelProvider;

import android.os.Bundle;

import androidx.annotation.NonNull;
import androidx.annotation.Nullable;
import androidx.fragment.app.Fragment;

import android.view.LayoutInflater;
import android.view.View;
import android.view.ViewGroup;

public class EndPage extends WizardPage {

    private EndPageViewModel mViewModel;

    public static EndPage newInstance() {
        return new EndPage();
    }

    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
                             @Nullable Bundle savedInstanceState) {

        View view = inflater.inflate(R.layout.fragment_end_page, container, false);

        view.findViewById(R.id.endButton).setOnClickListener((View v) -> {
            ((MainActivity) getActivity()).wizard.restart();
        });

        return view;
    }

    @Override
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        super.onActivityCreated(savedInstanceState);
        mViewModel = new ViewModelProvider(this).get(EndPageViewModel.class);
        // TODO: Use the ViewModel
    }

    @Override
    public boolean getNavigationVisibility() {
        return false;
    }

    @Override
    public boolean getNextEnabled() {
        return false;
    }

    @Override
    public Class<? extends Fragment> saveAndStep(WizardData data) {
        return null;
    }
}